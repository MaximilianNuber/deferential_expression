...
class RESummarizedExperiment(SummarizedExperiment): # type: ignore[misc]
    ...
    def set_row_names(self, names: List[str]):
        """Set row names of the RESummarizedExperiment, also setting the row names of all R-matrices inside the assay dictionary"""
        assays = self._assays
        ...
        for assay in assay_names:
            ...
            mat_newnames = set_row_names(mat, list(names))

            new_assays[assay] = RMatrixAdapter(mat_newnames)

        super().set_row_names(names)
