...
`deferential_expression` relies on R and specific Bioconductor packages (`edgeR`, `limma`). You can install these dependencies automatically using Conda (recommended) or manually using a system-installed R.
...
3.  Install the package in editable mode (optional, if developing):

    ```bash
    pip install -e .
    ```

This method automatically handles the installation of `r-base`, `rpy2`, `edgeR`, and `limma`.
...
2.  **Automatic R Package Installation**:
    When you first run the package, it will attempt to detect if `edgeR` and `limma` are installed in your R library. If they are missing, it will attempt to install them using `BiocManager`.
...
Add `rpy2` to your `setup.cfg` so it is installed automatically by pip.
